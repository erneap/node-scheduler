...
import { NewCompany, Team, UpdateTeam } from "scheduler-models/scheduler/teams";
...
router.post('/team/company', auth, async(req: Request, res: Response) => {
  try {
    ...
    await postLogEntry('team', `teamCompany: Post: Error: ${error.message}`);
...
router.put('/team/company', auth, async(req: Request, res: Response) => {
  try {
    ...
    const error = err as Error;
    await postLogEntry('team', `teamCompany: Put: Error: ${error.message}`);
    res.status(400).json({'message': error.message});
  }
});
...
router.delete('/team/company/:team/:company', auth, async(req: Request, res: Response) => {
  try {
    ...
    const error = err as Error;
    await postLogEntry('team', `teamCompany: Delete: Error: ${error.message}`);
    res.status(400).json({'message': error.message});
  }
});
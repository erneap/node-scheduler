...
@Component({
  selector: 'app-team-editor-specialties',
  imports: [
    List,
    FormField,
    MatFormFieldModule,
  ...
  templateUrl: './team-editor-specialties.html',
  styleUrl: './team-editor-specialties.scss',
})
export class TeamEditorSpecialties {
  team = signal<string>('');
  list = signal<Item[]>([]);
  selectedItem = signal<string>('new');
  contactModel = signal<SpecialtyData>({
    name: ''
  });
  ...
  setContactInfoList() {
    const list: Item[] = [];
    list.push({id: 'new', value: 'Add New Specialty Type'});
    const iTeam = this.teamService.getTeam();
    if (iTeam) {
      const team = new Team(iTeam);
      team.specialties.forEach(ct => {
        list.push({id: `${ct.id}`, value: ct.name});
      });
      this.contactsLength.set(team.specialties.length);
    }
    this.list.set(list);
  }
  ...
  onUpdate(field: string) {
    if (this.selectedItem().toLowerCase() !== 'new') {
      ...
      switch (field.toLowerCase()) {
        case "name":
          useField = 'changespecialty';
          value = this.contactForm.name().value();
          break;
        case "up":
        case "down":
          useField = 'movespecialty';
          value = field;
          break;
      }
  ...
  onDelete() {
    const dialogRef = this.dialog.open(ConfirmationDialog, {
      data: {
        title: 'Specialty Type Confirmation',
        message: 'Are you sure you want to delete this Specialty Type?',
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result.toLowerCase() === 'yes') {
        const useField = 'removespecialty';
        const value = '';
        this.teamService.updateTeam(this.team(), useField, value, this.selectedItem())
          .subscribe({
          next: (res) => {
